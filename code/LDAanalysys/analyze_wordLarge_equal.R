...
for(j in 1:length(id)){
  setwd(wd1)
  doc=read.csv(paste("Data/Language_Large_Equal/corpus",id[j],".csv",sep=""),header=FALSE)
  ...
  alpha=read.csv(paste("Data/Language_Large_Equal/theta",id[j],".csv",sep=""),header=FALSE)
  D=dim(tfm)[1]
  gamma=list()
...
setwd(wd1)
write.csv(accur,"Results/accur_lare_equal3.csv")
write.csv(repro,"Results/repro_lare_equal3.csv")
