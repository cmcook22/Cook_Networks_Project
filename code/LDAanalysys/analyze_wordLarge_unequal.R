...
id=c(1000,5000,10000)
runs=100
K=10

for(j in 1:length(id)){
  setwd(wd1)
  doc=read.csv(paste("Data/Language_Large_Unequal/corpus",id[j],".csv",sep=""),header=FALSE)
  ...
  alpha=read.csv(paste("Data/Language_Large_Unequal/theta",id[j],".csv",sep=""),header=FALSE)
  D=dim(tfm)[1]
  gamma=list()
...
setwd(wd1)
write.csv(accur,"Results/accur_lare_unequal3.csv")
write.csv(repro,"Results/repro_lare_unequal3.csv")
