...
#Edge List for Word Network  
  ...
  weight2=read.csv("SciERGM/network2Sci.csv")[,-1]
...
############################################################################
  ...
  x=matrix(0,nrow=1995,ncol=1995)
  for(i in 1:1995){
    x[i,]=rep(clust[i],1995)
  }
  x=x-t(x)
  x[x>0]=1
  x[x<0]=1
  ...
  pdf("sciencePlot.pdf",height=10,width=10, pointsize=8)
  # set some graphical parameters (see ?par)
  par(las=1,mar=c(3.25,4,1,1))
  # Simple plot
